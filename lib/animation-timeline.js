  ...
  timeline_createClass(Timeline, [{
    ...
    value: function _moveElements(offset, elements) {
      ...
      if (Math.abs(offset) > 0) {
        ...
        bounds = this._setMinMax(bounds, this._options); // find allowed min bounds for the draggable items

        elements.forEach(function (p) {
          bounds = _this2._setMinMax(bounds, p.keyframe);
          bounds = _this2._setMinMax(bounds, p.row);
        });
        elements.forEach(function (p) {
      ...
      rows.forEach(function (row, index) {
        ...
        var calcRow = {
          size: {
            x: 0,
            y: currentRowY,
            width: _this6._canvas ? _this6._canvas.clientWidth : 0,
            height: rowHeight
          },
        ...
        calcRow.groups.forEach(function (group) {
          ...
          _this6._setMinMax(calcRow, group); // get group screen coords
        ...
        _this6._setMinMax(toReturn, calcRow);
      });
      ...
      this._forEachKeyframe(function (calcKeyframe) {
        ...
        if (pos) {
          ...
          if (bounds && bounds.overlapY) {
            ...
            _this8._ctx.rect(0, timelineStyleUtils_TimelineStyleUtils.headerHeight(_this8._options), _this8._canvas.clientWidth, _this8._canvas.clientWidth);
          ...
          var keyframeColor = keyframe.selected ? timelineStyleUtils_TimelineStyleUtils.keyframeFillColor(keyframe, row, _this8._options) : timelineStyleUtils_TimelineStyleUtils.keyframeSelectedFillColor(keyframe, row, _this8._options);
          var border = timelineStyleUtils_TimelineStyleUtils.keyframeStrokeThickness(keyframe, row, _this8._options);
          var strokeColor = border > 0 ? timelineStyleUtils_TimelineStyleUtils.keyframeStrokeColor(keyframe, row, _this8._options) : '';
