...
var timeline_Timeline = /*#__PURE__*/function (_TimelineEventsEmitte) {
  ...
  timeline_createClass(Timeline, [{
    ...
    value: function _forEachKeyframe(callback) {
      var calculateGroupsBounds = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : false;

      if (!this._model) {
        return;
      }

      var model = this._calculateRowsBounds(calculateGroupsBounds);
    ...
    value: function _calculateRowsBounds() {
      ...
      rows.filter(function (p) {
        ...
        if (!isNaN(rowData.minValue) && !isNaN(rowData.maxValue)) {
          ...
          var groupRect = _this5._getKeyframesGroupSize(row, rowData.y, rowData.minValue, rowData.maxValue);
    ...
    value: function _renderRows() {
      ...
      if (data && data.rows) {
        ...
        data.rows.forEach(function (rowData) {
          ...
          var keyframeLaneColor = TimelineStyleUtils.groupFillColor(rowData.row, _this6._options);
          ...
          var rectBounds = _this6._cutBounds(rowData.groupRect);
    ...
    value: function _findDraggable(elements) {
      ...
      var filteredElements = elements.filter(function (element) {
        ...
        if (element.type === TimelineElementType.Keyframe) {
          ...
          if (!TimelineStyleUtils.groupDraggable(element.row, _this8._options)) {
            return false;
          }
      ...
      if (pos.y >= this._options.headerHeight && this._options.keyframesDraggable) {
        this._forEachKeyframe(function (keyframe, keyframeIndex, rowModel, rowIndex, isNextRow) {
          ...
          if (isNextRow && rowModel.groupRect) {
            ...
            var keyframesGroupOverlapped = TimelineUtils.isOverlap(pos.x, pos.y, rowModel.groupRect);
