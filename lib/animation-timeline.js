...
var timelineStyleUtils_TimelineStyleUtils = /*#__PURE__*/function () {
  ...
  timelineStyleUtils_createClass(TimelineStyleUtils, null, [{
    ...
    value: function groupDraggable(rowStyle, options) {
      ...
      return TimelineStyleUtils.getRowStyle(rowStyle, options, 'groupDraggable', defaultValue, true);
    }
  }]);
...
var TimelineSelectedEvent = /*#__PURE__*/function (_TimelineBaseEvent) {
  ...
  function TimelineSelectedEvent() {
    ...
    timelineSelectedEvent_defineProperty(_assertThisInitialized(_this), "keyframes", void 0);
...
var timeline_Timeline = /*#__PURE__*/function (_TimelineEventsEmitte) {
  ...
  timeline_createClass(Timeline, [{
    ...
    value: function selectAllKeyframes() {
      var changed = this._performSelection(true);
    ...
    value: function getSelectedElements() {
      ...
      this._forEachKeyframe(function (keyframe) {
        if (keyframe && keyframe.model.selected) {
          selected.push(_this2._convertToElement(keyframe.parentRow.model, keyframe.model));
        }
    ...
    value: function _forEachKeyframe(callback) {
      ...
      model.rows.forEach(function (calcRow) {
        if (!calcRow) {
          return;
        }
      ...
      if (this._consts.autoPanSpeed) {
        if (!this._intervalRef) {
          ...
          this._intervalRef = setInterval(function () {
            _this3._handleMouseMoveEvent(null);
    ...
    value: function _setMinMax(to, from) {
      ...
      if (to.minValue !== null && from.minValue !== null) {
        var min = Math.min(from.minValue, to.minValue);
        to.minValue = min;
    ...
    value: function _calculateModel() {
      ...
      rows.forEach(function (row, index) {
        ...
        var rowHeight = timelineStyleUtils_TimelineStyleUtils.getRowHeight(row, _this4._options);
        var marginBottom = timelineStyleUtils_TimelineStyleUtils.getRowMarginBottom(row, _this4._options);
        var currentRowY = rowAbsoluteHeight - _this4._scrollContainer.scrollTop;
        ...
        var rowSize = {
          x: 0,
          y: currentRowY,
          width: _this4._canvas.clientWidth,
          height: rowHeight
        };
        var calcRow = {
          size: rowSize,
        ...
        if (row && row.keyframes) {
          row.keyframes.forEach(function (keyframe) {
            if (keyframe && !isNaN(keyframe.val) && !keyframe.hidden) {
              ...
              var keyframeSize = _this4._getKeyframePosition(keyframe, calcRow);
              ...
              var min = currentGroup.minValue == null ? keyframe.val : Math.min(keyframe.val, currentGroup.minValue);
              var max = currentGroup.maxValue == null ? keyframe.val : Math.max(keyframe.val, currentGroup.maxValue);
        ...
        calcRow.groups.forEach(function (group) {
          ...
          _this4._setMinMax(calcRow, group); // get group screen coords


          var groupRect = _this4._getKeyframesGroupSize(row, rowSize.y, group.minValue, group.maxValue);

          group.size = groupRect;
        }); // Extend screen bounds by a current calculation:

        _this4._setMinMax(toReturn, calcRow);
      });
    ...
    value: function _renderRows() {
      ...
      if (data && data.rows) {
        ...
        data.rows.forEach(function (rowCalc) {
          ...
          _this5._ctx.fillStyle = timelineStyleUtils_TimelineStyleUtils.getRowFillColor(rowCalc.model, _this5._options); //this._ctx.fillRect(data.areaRect.x, data.areaRect.y, data.areaRect.w, data.areaRect.h);
          // Note: bounds used instead of the clip while clip is slow!

          var bounds = _this5._cutBounds(rowCalc.size);

          if (bounds) {
            _this5._ctx.fillRect(bounds.x, bounds.y, bounds.width, bounds.height);
          }

          var keyframeLaneColor = timelineStyleUtils_TimelineStyleUtils.groupFillColor(rowCalc.model, _this5._options);
          ...
          rowCalc.groups.forEach(function (group) {
            ...
            var rectBounds = _this5._cutBounds(group.size);

            if (rectBounds) {
              _this5._ctx.fillStyle = keyframeLaneColor;

              _this5._ctx.fillRect(rectBounds.x, rectBounds.y, rectBounds.width, rectBounds.height);
            }
          });
        });
    ...
    value: function _renderKeyframes() {
      ...
      this._forEachKeyframe(function (calcKeyframe) {
        ...
        if (pos) {
          var x = _this6._getSharp(pos.x);

          var y = pos.y;

          var bounds = _this6._cutBounds({
            x: x - pos.width / 2,
            y: y - pos.height / 2,
            width: pos.width,
            height: pos.height
          ...
          _this6._ctx.save(); // Performance FIX: use clip only  when we are in the collision! Clip is slow!
          ...
          if (bounds && bounds.overlapY) {
            ...
            _this6._ctx.rect(0, _this6._options.headerHeight || 0, _this6._canvas.clientWidth, _this6._canvas.clientWidth);

            _this6._ctx.clip();
          }

          var shape = timelineStyleUtils_TimelineStyleUtils.keyframeShape(keyframe, row, _this6._options);

          if (shape === TimelineKeyframeShape.None) {
            return;
          }

          var keyframeColor = keyframe.selected ? timelineStyleUtils_TimelineStyleUtils.keyframeFillColor(keyframe, row, _this6._options) : timelineStyleUtils_TimelineStyleUtils.keyframeSelectedFillColor(keyframe, row, _this6._options);
          var border = timelineStyleUtils_TimelineStyleUtils.keyframeStrokeThickness(keyframe, row, _this6._options);
          var strokeColor = border > 0 ? timelineStyleUtils_TimelineStyleUtils.keyframeStrokeColor(keyframe, row, _this6._options) : '';

          if (shape == TimelineKeyframeShape.Rhomb) {
            _this6._ctx.beginPath();

            _this6._ctx.translate(x, y);

            _this6._ctx.rotate(45 * Math.PI / 180);

            if (border > 0 && strokeColor) {
              _this6._ctx.fillStyle = strokeColor;

              _this6._ctx.rect(-pos.width / 2, -pos.height / 2, pos.width, pos.height);

              _this6._ctx.fill();
            }

            _this6._ctx.fillStyle = keyframeColor; // draw main keyframe data with offset.

            _this6._ctx.translate(border, border);

            _this6._ctx.rect(-pos.width / 2, -pos.height / 2, pos.width - border * 2, pos.height - border * 2);
            ...
            if (border > 0 && strokeColor) {
              ...
              _this6._ctx.arc(x, y, pos.height, 0, 2 * Math.PI);
            }

            _this6._ctx.fillStyle = keyframeColor;

            _this6._ctx.arc(x, y, pos.height - border, 0, 2 * Math.PI);
            ...
            if (border > 0 && strokeColor) {
              ...
              _this6._ctx.rect(x, y, pos.width, pos.height);
            ...
            _this6._ctx.rect(x + border, y + border, pos.width - border, pos.height - border);
    ...
    value: function _findDraggable(elements) {
      ...
      var filteredElements = elements.filter(function (element) {
        ...
        if (element.type === TimelineElementType.Keyframe) {
          if (!timelineStyleUtils_TimelineStyleUtils.keyframeDraggable(element.keyframe, element.row, _this7._options)) {
            return false;
          }
        } else if (element.type === TimelineElementType.Group) {
          if (!timelineStyleUtils_TimelineStyleUtils.groupDraggable(element.row, _this7._options)) {
            return false;
          }
      ...
      if (pos.y >= this._options.headerHeight && this._options.keyframesDraggable) {
        this._forEachKeyframe(function (calcKeyframe, index, isNextRow) {
          ...
          if (isNextRow) {
            ...
            if (rowOverlapped) {
              var row = {
                val: _this8._mousePosToVal(pos.x, true),
            ...
            if (calcKeyframe.parentRow.groups) {
              calcKeyframe.parentRow.groups.forEach(function (group) {
                ...
                if (keyframesGroupOverlapped) {
                  var keyframesModels = [];
                  var calcKeyframes = group.keyframes;

                  for (var i = 0; i < calcKeyframes.length; i++) {
                  ...
                  var snapped = _this8.snapVal(group.minValue); // get snapped mouse pos based on a min value.
...
var timelineElement = __webpack_require__(9);
...
var timelineScrollEvent = __webpack_require__(12);
