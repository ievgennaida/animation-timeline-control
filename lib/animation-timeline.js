  ...
  timeline_createClass(Timeline, [{
    ...
    value: function _selectInternal(nodes) {
      ...
      if (nodesArray && mode === TimelineSelectionMode.Append) {
        nodes.forEach(function (node) {
          var changed = _this3._changeNodeState(state, node, true);
        ...
        nodes.forEach(function (node) {
          if (state.selected.indexOf(node) >= 0) {
            _this3._changeNodeState(state, node, false);

            TimelineUtils.deleteElement(state.selected, node);
          } else {
            _this3._changeNodeState(state, node, true);
    ...
    value: function _renderTicks() {
      ...
      for (var i = from; i <= to; i += step) {
        ...
        TimelineUtils.drawLine(this._ctx, sharpPos, timelineStyleUtils_TimelineStyleUtils.headerHeight(this._options) / 2, sharpPos, this._canvas.clientHeight);
    ...
    value: function _calculateModel() {
      ...
      rows.forEach(function (row, index) {
        ...
        var rowHeight = timelineStyleUtils_TimelineStyleUtils.getRowHeight(row, _this5._options);
        var marginBottom = timelineStyleUtils_TimelineStyleUtils.getRowMarginBottom(row, _this5._options);
        var currentRowY = rowAbsoluteHeight - (_this5._scrollContainer ? _this5._scrollContainer.scrollTop : 0);
        ...
        var calcRow = {
          size: {
            x: 0,
            y: currentRowY,
            width: _this5._canvas ? _this5._canvas.clientWidth : 0,
            height: rowHeight
          },
        ...
        if (row && row.keyframes) {
          row.keyframes.forEach(function (keyframe) {
            if (keyframe && !isNaN(keyframe.val) && !keyframe.hidden) {
              ...
              var keyframeSize = _this5._getKeyframePosition(keyframe, calcRow);
        ...
        calcRow.groups.forEach(function (group) {
          ...
          _this5._setMinMax(calcRow, group); // get group screen coords


          var groupRect = _this5._getKeyframesGroupSize(row, calcRow.size.y, group.minValue, group.maxValue);

          group.size = groupRect;
        }); // Extend screen bounds by a current calculation:

        _this5._setMinMax(toReturn, calcRow);
      });
    ...
    value: function _renderRows() {
      ...
      if (data && data.rows) {
        ...
        data.rows.forEach(function (rowCalc) {
          ...
          _this6._ctx.fillStyle = timelineStyleUtils_TimelineStyleUtils.getRowFillColor(rowCalc.model, _this6._options); //this._ctx.fillRect(data.areaRect.x, data.areaRect.y, data.areaRect.w, data.areaRect.h);
          // Note: bounds used instead of the clip while clip is slow!

          var bounds = _this6._cutBounds(rowCalc.size);

          if (bounds) {
            _this6._ctx.fillRect(bounds.x, bounds.y, bounds.width, bounds.height);
          }

          var keyframeLaneColor = timelineStyleUtils_TimelineStyleUtils.groupFillColor(rowCalc.model, _this6._options);
          ...
          rowCalc.groups.forEach(function (group) {
            ...
            var rectBounds = _this6._cutBounds(group.size);
      ...
      this._forEachKeyframe(function (calcKeyframe) {
        ...
        if (pos) {
          ...
          if (bounds && bounds.overlapY) {
            ...
            _this7._ctx.rect(0, timelineStyleUtils_TimelineStyleUtils.headerHeight(_this7._options), _this7._canvas.clientWidth, _this7._canvas.clientWidth);

            _this7._ctx.clip();
          }

          var shape = timelineStyleUtils_TimelineStyleUtils.keyframeShape(keyframe, row, _this7._options);

          if (shape === TimelineKeyframeShape.None) {
            return;
          }

          var keyframeColor = keyframe.selected ? timelineStyleUtils_TimelineStyleUtils.keyframeFillColor(keyframe, row, _this7._options) : timelineStyleUtils_TimelineStyleUtils.keyframeSelectedFillColor(keyframe, row, _this7._options);
          var border = timelineStyleUtils_TimelineStyleUtils.keyframeStrokeThickness(keyframe, row, _this7._options);
          var strokeColor = border > 0 ? timelineStyleUtils_TimelineStyleUtils.keyframeStrokeColor(keyframe, row, _this7._options) : '';
          ...
          if (shape == TimelineKeyframeShape.Rhomb) {
            ...
            if (border > 0 && strokeColor) {
              ...
              _this7._ctx.rect(-pos.width / 2, -pos.height / 2, pos.width, pos.height);
            ...
            _this7._ctx.rect(-pos.width / 2, -pos.height / 2, pos.width - border * 2, pos.height - border * 2);
            ...
            if (border > 0 && strokeColor) {
              ...
              _this7._ctx.arc(x, y, pos.height, 0, 2 * Math.PI);
            }

            _this7._ctx.fillStyle = keyframeColor;

            _this7._ctx.arc(x, y, pos.height - border, 0, 2 * Math.PI);
            ...
            if (border > 0 && strokeColor) {
              ...
              _this7._ctx.rect(x, y, pos.width, pos.height);
            ...
            _this7._ctx.rect(x + border, y + border, pos.width - border, pos.height - border);
    ...
    value: function _renderHeaderBackground() {
      ...
      if (timelineStyleUtils_TimelineStyleUtils.headerHeight(this._options)) {
        ...
        if (this._options.headerFillColor) {
          ...
          this._ctx.fillRect(0, 0, this._canvas.clientWidth, timelineStyleUtils_TimelineStyleUtils.headerHeight(this._options));
        } else {
          this._ctx.clearRect(0, 0, this._canvas.clientWidth, timelineStyleUtils_TimelineStyleUtils.headerHeight(this._options));
        }
    ...
    value: function _findDraggable(elements) {
      ...
      var filteredElements = elements.filter(function (element) {
        ...
        if (element.type === TimelineElementType.Keyframe) {
          if (!timelineStyleUtils_TimelineStyleUtils.keyframeDraggable(element.keyframe, element.row, _this8._options)) {
            return false;
          }
        } else if (element.type === TimelineElementType.Group) {
          if (!timelineStyleUtils_TimelineStyleUtils.groupDraggable(element.row, _this8._options)) {
            return false;
          }
            ...
            if (calcKeyframe.parentRow.groups) {
              calcKeyframe.parentRow.groups.forEach(function (group) {
                ...
                if (keyframesGroupOverlapped) {
                  var keyframesModels = _this9._mapKeyframes(group.keyframes);
                  ...
                  var snapped = _this9.snapVal(group.minValue); // get snapped mouse pos based on a min value.
...
var timelineSelectionResults = __webpack_require__(12);
